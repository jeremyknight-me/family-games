...
{
	private readonly WinningCombinations winCombinations;

    private Board()
    {
		this.winCombinations = WinningCombinations.Create();
    }

    public byte[] Cells { get; private set; } = new byte[42];
	public string? CurrentMessage => (this.WinnerMessage == string.Empty) ? $"Player {this.PlayerTurn}'s Turn" : "";
	...
	public byte PlayerTurn => (byte)(this.Cells.Count(x => x != 0) % 2 + 1);
	...
	public void PlayPiece(byte column)
	{
		...
		try
		{
			...
			// Check the column
			if (this.Cells[column] != 0) throw new ArgumentException("Column is full");

			var landingRow = this.DropPiece(column);
	...
	private byte DropPiece(byte column)
	{
		// Drop the piece in
		...
		return this.ConvertLandingSpotToRow(landingSpot);
	}
	...
	private MoveResult CheckForWin()
	{
		...
		foreach (var scenario in this.winCombinations.Combinations)
		{
